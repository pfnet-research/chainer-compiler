...
class E2E(chainer.Chain):
    ...
    def forward(self, xs, ilens, ys):
        ...
        # とりあえずCTCは無視しているので、 loss_ctc は0でよい
...
# from https://github.com/espnet/espnet/blob/master/src/nets/e2e_asr.py
...
def make_arg(**kwargs):
    defaults = dict(
        ...
        aconv_chans=10,
        aconv_filts=100,
        mtlalpha=0, #とりあえず、ctc側は通らないパスを行く
        lsm_type="",
        lsm_weight=0.0,
        adim=320,
        dropout_rate=0.0,
        ...
        minlenratio=0.0,
        ctc_weight=0.2,
        verbose=0, #とりあえず verbose=0 で許して 
...
if __name__ == '__main__':
    ...
    # 参考
    # https://github.com/espnet/espnet/blob/master/test/test_e2e_asr.py 
    # https://github.com/espnet/espnet/blob/master/test/test_recog.py
    args = make_arg()
    
    # https://github.com/espnet/espnet/blob/master/src/asr/asr_chainer.py#L245 trainはここ
    
    # https://github.com/espnet/espnet/blob/master/src/asr/asr_chainer.py#L286 data はここ
    
    # https://github.com/espnet/espnet/blob/master/test/test_e2e_asr.py#L55 これがまさにテストデータ生成なのでかち
    n_batch = 3
    max_x_len = 5
    n_ylen = 7
    
    misc = 10
    model = E2E(misc,5,args)
    
    ilens = [3,5,7]
    olens = [4,2,6]
    
    assert len(ilens) == len(olens)
    xs = [np.random.randn(ilen, misc).astype(np.float32) for ilen in ilens]

...
// Ther order must be specified in the variable `order_string`.
// `order_string` must be a comma-separated string that represents a sequence of orders.
...
std::vector<Order> CustomPolicy(const Graph& graph, std::string order_string) {
    CHECK(!order_string.empty()) << "order_string must be specified.";
    std::stringstream ss(order_string);
    std::string token;

    std::vector<Order> orders;
    while (std::getline(ss, token, ',')) {
        CHECK_GE(token.size(), 3) << "Invalid token in order_string: " << token;
